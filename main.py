...
# ---------------------------------------------------------
# 설정 (본인의 GAS 웹앱 URL을 입력하세요)
# ---------------------------------------------------------
GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbxnfkeDRnwvdVT5_KDOjpy66u5y0JJqIOFteNyArvaXCDEN1tKq5GOujIrjjL8IcArSpA/exec"
...
def fetch_target_list():
    ...
    try:
        response = requests.get(GAS_WEBAPP_URL, params={"action": "getSsuSponTargetList"}, timeout=10)
...
def parse_spon_table(soup):
    ...
    """
    raw_months = defaultdict(lambda: {
        "total_match": 0, "total_win": 0, "total_lose": 0,
        "tribe": {
            "Z": {"match": 0, "win": 0, "lose": 0},
            "P": {"match": 0, "win": 0, "lose": 0},
            "T": {"match": 0, "win": 0, "lose": 0}
        },
        "mini_match": 0, "mini_win": 0, "mini_lose": 0
    })
    ...
    for row in rows:
        ...
        # ★ 키 포맷 변경: 1~9월을 01~09 두 자리로 생성하여 정렬 문제 해결 (예: "2025-01")
        ...
        is_mini = ("미니대전" in match_type_text) or ("미니대학대전" in match_type_text)
    ...
    # 포맷 정형화 (스폰수, 승, 패, 승률 계산) - sorted(raw_months.items())로 날짜 오름차순 정렬
    formatted_result = {}
        ...
        formatted_result[month_key] = {
            ...
            "종족별": {
                t: {
                    ...
                    "승": data["tribe"][t]["win"],
                    "패": data["tribe"][t]["lose"],
                    "승률": round((data["tribe"][t]["win"] / data["tribe"][t]["match"] * 100), 1) if data["tribe"][t]["match"] > 0 else 0.0
                } for t in ["Z", "P", "T"]
            },
            "미니대전": {
                "스폰수": mini_m,
                "승": mini_w,
                "패": mini_l,
                "승률": round((mini_w / mini_m * 100), 1) if mini_m > 0 else 0.0
            }
        }
...
def crawl_spon_data(target_item, current_idx, total_count):
    """requests로 빠른 HTML 수집 및 파싱 (진행도 표시 포함)"""
    url = target_item["sponUrl"]
    streamer_name = target_item.get("streamerName", "알 수 없음")
    
    print(f"[{current_idx}/{total_count}] [{streamer_name}] 데이터 요청 중: {url}")
    ...
    try:
        ...
        if not parsed_json_data:
            print(f" -> 전적 데이터 없음 또는 최근전적 목록 비어있음")
        else:
            print(f" -> 성공: {len(parsed_json_data)}개 월별 데이터 파싱완료")
...
def send_payload_to_gas(payload):
    ...
    body = {
        "action": "updateSsuSponData",
        "payload": payload
    }
    try:
        response = requests.post(GAS_WEBAPP_URL, data=json.dumps(body), headers=headers, timeout=15)
...
def main():
    ...
    for idx, target in enumerate(targets, 1):
        spon_data = crawl_spon_data(target, idx, total_count)
